    ...
    class TelnetServer : TCPServer
    {
                ...
                if (bytes[i] == (byte)NegociationOptions.WILL)
                {
                    if (length > i + 1)
                    {
                        ...
                        else if (bytes[i + 1] == (byte)NegociationOptions.NEWENVIRON)
                        {
                            ...
                            stream.WriteByte((byte)NegociationOptions.NEWENVIRON);
                            stream.WriteByte((byte)NegociationOptions.SEND);
                            stream.WriteByte((byte)NegociationOptions.IAC);
                        ...
                        else if (bytes[i + 1] == (byte)NegociationOptions.TTYPE)
                        {
                            ...
                            stream.WriteByte((byte)NegociationOptions.IAC);
                            stream.WriteByte((byte)NegociationOptions.SB);
                            stream.WriteByte((byte)NegociationOptions.TTYPE);
                            stream.WriteByte((byte)NegociationOptions.SEND);
                            stream.WriteByte((byte)NegociationOptions.IAC);
                            stream.WriteByte((byte)NegociationOptions.SE);
                        }
                        else if (bytes[i + 1] == (byte)NegociationOptions.CHARSET)
                        {
                            ...
                            byte[] charsetsupport = System.Text.Encoding.ASCII.GetBytes(";UTF-8;US-ASCII");
                            stream.WriteByte((byte)NegociationOptions.IAC);
                            stream.WriteByte((byte)NegociationOptions.SB);
                ...
                else if (bytes[i] == (byte)NegociationOptions.SB)
                {
                    ...
                    if (length > i + 1)
                    {
                        ...
                        else if (bytes[i + 1] == (byte)NegociationOptions.TTYPE)
                        {
                            ...
                            if (bytes[i + 2] != (byte)NegociationOptions.IS) break;
                            StringBuilder TermType = new StringBuilder();
                            ...
                            for (i = i + 3; i < length; i++)
                            {
                                    ...
                                    bytes[i + 1] == (byte)NegociationOptions.SE)
                                {
                                    ...
                                    {
                                        client.KeyValuePairs["TERMTYPE"] = " " + TermType.ToString();
                                    }
                                    completed = true;
                                    i++;
                                }
                        ...
                        {
                            StringBuilder charsetnegociation = new StringBuilder();
        ...
        public override void Startup()
        {
            try
            {
                ...
                Byte[] bytes = new Byte[2097152]; // 2 megabytes large. This is in the case of POST data!
                String data = null;
                ...
                while (true)
                {
                    ...
                    TCPInitializer.Client client = new TCPInitializer.Client(AcceptTcpClient(), Metadata);
                    ...
                    foreach (var option in new[] {NegociationOptions.NAWS, 
                    ...
                    while ((i = stream.Read(bytes, 0, bytes.Length)) != 0)
                    {
                        ...
                        StringBuilder bytesgotten = new StringBuilder();
                        ...
                        data = System.Text.Encoding.ASCII.GetString(bytes, negstart, i - negstart);
