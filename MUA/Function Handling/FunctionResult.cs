...
namespace MUA
{
    ...
    ///     function.
    ///     <remarks>
    ///         This by-steps the buffer requirements, until we get to the end. We're passing around the same object in a
    ///         chain, editing the same object, thus reducing the strain this might have otherwise - if we had to constantly
    ///         copy objects.
    ...
    public class FunctionResult
    {
        ...
        private void CopyResult(FunctionResult result)
        {
            List<MarkupString> markupListCopy = result.GetResults().Select(each => new MarkupString(each)).ToList();

            // Makes a copy of each item in 'result', and then stores it in this instance's own results.
            this.results.Clear();
            this.results.AddRange(markupListCopy);
        }
    }
}